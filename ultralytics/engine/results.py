...
class Results(SimpleClass):
    ...
    def save_counts(result, count_file):
        """
        统计每个标签的数量并写入一个txt文件，并打印调试信息。
        
        Args:
            result: YOLO模型预测的Result对象。
            count_file: 统计结果要保存的txt文件路径。
        """
        ...
        # 打印调试信息
        print(f"[DEBUG] 图像路径: {result.path}")
        if label_counts:
            print("[DEBUG] 识别结果:")
            for label, count in label_counts.items():
                print(f"         {label}: {count}")
        else:
            print("[DEBUG] 未检测到任何标签")

        # 准备写入内容
        count_lines = [f"{result.path}:\n"]
        for label, count in label_counts.items():
        ...
        # 文件写入调试输出
        print(f"[DEBUG] 写入统计结果到: {count_file}\n")
    ...
    def save_txt(self, txt_file, save_conf=False):
        ...
        count_file = Path(txt_file).with_name("label_counts.txt")
        self.save_counts(self, count_file)
